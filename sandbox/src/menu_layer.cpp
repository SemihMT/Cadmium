...
namespace Sandbox
{
  ...
  {
#ifdef CADMIUM_IMGUI
      float w = std::max(static_cast<float>(GetWidth()), 400.0f);
      float h = std::max(static_cast<float>(GetHeight()), 400.0f);
      float x = GetViewport().GetScreenX();
      float y = GetViewport().GetScreenY();
