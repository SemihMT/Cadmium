...
namespace Sandbox
{
  ...
  void WorldLayer::OnAttach()
  {
    ...
    m_DebrisToken = Subscribe<AsteroidDestroyedEvent>([this](const AsteroidDestroyedEvent &e)
                                                      {
      auto& world = GetWorld();
      int count   = (2 - e.generation) * 3 + 3; // more debris from larger asteroids
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        // Color by generation
        Debris debris;
        debris.radius   = RandomFloat(2.0f, 5.0f);
        debris.lifetime = RandomFloat(1.0f, 2.5f);
        if (e.generation == 0)      { debris.r = 0.9f; debris.g = 0.6f; debris.b = 0.3f; }
        else if (e.generation == 1) { debris.r = 0.5f; debris.g = 0.9f; debris.b = 0.5f; }
        else                        { debris.r = 0.5f; debris.g = 0.6f; debris.b = 0.9f; }

        world.AddComponent<Debris>(entity, debris);
  } });
  ...
  {
    auto &s = *m_State;
    if (s.asteroidCount >= GameState::k_MaxAsteroids)
      return;
  ...
  void WorldLayer::CheckCollisions()
  {
    ...
    for (auto &a : s.asteroids)
    {
      ...
      // Ship vs asteroid
      if (!s.ship.alive || s.ship.invincible)
        continue;
  ...
  void WorldLayer::FireBullet()
  {
    ...
    if (m_FireCooldown > 0.0f)
      return;
    if (s.bulletCount >= GameState::k_MaxBullets)
      return;
