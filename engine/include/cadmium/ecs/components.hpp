...
namespace Cadmium
{
    ...
    struct Transform
    {
        ...
        float GetRotationX() const { return glm::degrees(glm::eulerAngles(rotation).x); }
        float GetRotationY() const { return glm::degrees(glm::eulerAngles(rotation).y); }
        float GetRotationZ() const { return glm::degrees(glm::eulerAngles(rotation).z); }
        ...
        static Transform
        From2D(float x, float y, float rotationDegZ = 0.f, float scaleX = 1.f, float scaleY = 1.f)
        {
            Transform t;
            t.position = {x, y, 0.f};
        ...
        {
            rotation =
                glm::normalize(glm::angleAxis(glm::radians(deg), glm::normalize(axis)) * rotation);
        }
        ...
        glm::vec3 Forward() const { return rotation * glm::vec3(0.f, 0.f, -1.f); }
        glm::vec3 Right() const { return rotation * glm::vec3(1.f, 0.f, 0.f); }
        glm::vec3 Up() const { return rotation * glm::vec3(0.f, 1.f, 0.f); }
    };
    struct Velocity
    {
        float x{0.0f}, y{0.0f};
